...
pub enum SystemType {
    ...
    /// macOS with nix-darwin, activated with the `activate` script of its profile.
    Darwin,
}
...
impl SystemType {
    /// Whether nodes of this type can be deployed with `goal`.
    ///
    /// nix-darwin cannot activate at the next boot only, its activation
    /// script runs steps before the checks of a dry run, and the launchd
    /// plists it installs outlive a reboot, which would not undo `test`.
    pub fn supports(&self, goal: Goal) -> bool {
        match self {
            Self::NixOS => true,
            Self::Darwin => !matches!(goal, Goal::Boot | Goal::DryActivate | Goal::Test),
        }
    }

    /// Returns the directory to run Nix from, or `None` to find it through
    /// PATH.
    ///
    /// Root's non-interactive shell on macOS has no Nix in PATH, nor does
    /// sudo with `secure_path` set. The system profile has Nix when
    /// nix-darwin manages it (`nix.enable = true`).
    pub fn nix_bin_dir(&self) -> Option<&'static str> {
        match self {
            Self::NixOS => None,
            Self::Darwin => Some("/run/current-system/sw/bin"),
        }
    }
}
